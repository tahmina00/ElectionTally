...
router.put("/candidatebangla2api/:selectedElectionId", async (req, res) => {
  ...
  const { candidateData, constitutionsData } = updatedValues;
  ...
  constitutionsData.forEach(({ _id, totalcenter, obtainedcenter }) => {
    cons_objectIds.push(_id);
    cons_totalcenter.push(totalcenter);
    cons_obtainedcenter.push(obtainedcenter);
  });
...
const cons_updates = cons_objectIds.map((cons_objectIds, index) => ({
  updateOne: {
    filter: { _id: cons_objectIds },
    update: { $set: { totalcenter: cons_totalcenter[index], obtainedcenter: cons_obtainedcenter[index] } }
  }
}));
